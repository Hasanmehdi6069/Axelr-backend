...
async def call_ollama(model_name: str, prompt: str, max_tokens: int, temperature: float) -> Dict:
    import httpx
...
# ============================================================
async def route_to_model(workspace: str, payload: Dict) -> Dict:
    # Define model mapping
    model_map = {
        "data": {"primary": "phi3:3.8b-mini-4k-instruct-q4_K_M",
                 "secondary": "gemma2:2b-instruct-q4_K_M"},
        "design": {"primary": "qwen2.5-coder:1.5b-instruct-q4_K_M",
                   "secondary": "codegemma:2b-instruct-q4_K_M"}
    }
    tiers = ["primary", "secondary"]
    for tier in tiers:
        model = model_map.get(workspace, {}).get(tier)
        if not model:
            continue
        try:
            result = await call_ollama(model, payload["prompt"], payload["max_tokens"], payload["temperature"])
            return {
                "success": True,
            ...
            logger.warning(f"{tier} model {model} failed: {e}")
            continue
    # Fallback
    return {"success": False, "text": "All models unavailable.", "provider": "none", "model_used": "none", "tokens_used": 0}
# ============================================================
